...
var _ = Describe("SSH", func() {
	...
	verifyProxyHealthCheck := func(address string) {
		res, err := http.Get(address)
		Expect(err).NotTo(HaveOccurred())
		Expect(res.StatusCode).To(Equal(http.StatusOK))
	}

	verifyProxyHealthCheckFailure := func(address string) {
	...
	Context("when nothing is running", func() {
		It("should fail verifying the ssh proxy healthcheck", func() {
			verifyProxyHealthCheckFailure("http://" + componentMaker.Addresses.SSHProxyHealthCheck)
		})
	})

	Context("when rep is running", func() {
		BeforeEach(func() {
			processGuid = helpers.GenerateGuid()
			address = componentMaker.Addresses.SSHProxy
			healthCheckAddress = componentMaker.Addresses.SSHProxyHealthCheck
		...
		Context("when valid process guid and index are used in the username", func() {
			It("can respond to a healthcheck", func() {
				verifyProxyHealthCheck("http://" + healthCheckAddress)
			})
